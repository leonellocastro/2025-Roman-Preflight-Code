...
# --- Setup Paths ---
# (Paths remain as per your local setup)
pupil_path = "C:\\Users\\leone\\OneDrive\\Documents\\GitHub\\2025-Roman-Preflight-Code\\roman_preflight_proper_public_v2.0.1_python\\roman_preflight_proper\\preflight_data\\hlc_20190210b\\pupil.fits"
fpm_real_path = "C:\\Users\\leone\\OneDrive\\Documents\\GitHub\\2025-Roman-Preflight-Code\\roman_preflight_proper_public_v2.0.1_python\\roman_preflight_proper\\preflight_data\\hlc_20190210b\\hlc_fpm_trans_0.54625000um_real.fits"
fpm_imag_path = "C:\\Users\\leone\\OneDrive\\Documents\\GitHub\\2025-Roman-Preflight-Code\\roman_preflight_proper_public_v2.0.1_python\\roman_preflight_proper\\preflight_data\\hlc_20190210b\\hlc_fpm_trans_0.54625000um_imag.fits"
lyot_path = "C:\\Users\\leone\\OneDrive\\Documents\\GitHub\\2025-Roman-Preflight-Code\\roman_preflight_proper_public_v2.0.1_python\\roman_preflight_proper\\preflight_data\\hlc_20190210b\\lyot.fits"
...
grid_size = 1024
contrast = 1e0 # 10^0 contrast for visibility
sqrt_contrast = np.sqrt(contrast)

# 1. Initialize Wavefronts
wf_star = proper.prop_begin(diam, wavelength, grid_size, beam_ratio)
wf_planet = proper.prop_begin(diam, wavelength, grid_size, beam_ratio)

# 2. Planet Offset (Phase Ramp)
nw = proper.prop_get_gridsize(wf_planet)
sampling = proper.prop_get_sampling(wf_planet)
offset_x_lamD = 5.0
offset_y_lamD = 0.0
...
# IMPORTANT: Calculate ramp and shift center to [0,0]
phase_ramp = np.exp(2j * np.pi * (xx * offset_x_lamD / diam + yy * offset_y_lamD / diam))
proper.prop_multiply(wf_planet, np.fft.ifftshift(phase_ramp))
proper.prop_multiply(wf_planet, sqrt_contrast)

# 3. Combine and apply entrance pupil
wf = wf_star
wf.wfarr = wf_star.wfarr + wf_planet.wfarr

pupil_data = trim(fits.getdata(pupil_path), grid_size)
proper.prop_multiply(wf, np.fft.ifftshift(pupil_data))
proper.prop_define_entrance(wf)

# 4. Propagate to Focal Plane (FPM)
focal_length = 24 * diam 
proper.prop_lens(wf, focal_length, "to FPM")
proper.prop_propagate(wf, focal_length, "at FPM")

# Apply FPM
fpm_s = proper.prop_get_sampling(wf)
# Note: Ensure SAMPLING matches the FITS header pixel scale (m/pixel)
fpm_r = proper.prop_readmap(wf, fpm_real_path, 0, 0, AMPLITUDE=True, SAMPLING=fpm_s)
fpm_i = proper.prop_readmap(wf, fpm_imag_path, 0, 0, AMPLITUDE=True, SAMPLING=fpm_s)
fpm_complex = np.fft.ifftshift(fpm_r + 1j * fpm_i)
proper.prop_multiply(wf, fpm_complex)

# 5. Propagate to Lyot Plane
proper.prop_propagate(wf, focal_length, "after FPM")
proper.prop_lens(wf, focal_length, "to Lyot")
proper.prop_propagate(wf, focal_length, "at Lyot")

# Apply Lyot Stop
lyot_s = proper.prop_get_sampling(wf)
lyot_map = proper.prop_readmap(wf, lyot_path, 0, 0, AMPLITUDE=True, SAMPLING=lyot_s)
proper.prop_multiply(wf, np.fft.ifftshift(lyot_map))

# 6. Final Propagation to Detector
proper.prop_propagate(wf, focal_length, "after Lyot")
proper.prop_lens(wf, focal_length, "to Detector")
proper.prop_propagate(wf, focal_length, "at Detector")

# 7. Final Plotting
# Use fftshift for the display so the star is in the center, and log for dynamic range
final_psf = np.abs(np.fft.fftshift(wf.wfarr))**2
final_psf /= np.max(final_psf) # Normalize

plt.figure(figsize=(10, 8))
plt.imshow(np.log10(final_psf + 1e-12), origin="lower", extent=[-nw/2, nw/2, -nw/2, nw/2], cmap='magma')
plt.title(f"Final Detector Image (Planet at {offset_x_lamD} $\\lambda/D$)")
...
import proper
import astropy.io.fits as fits
from roman_preflight_proper import trim, polmap
import matplotlib.pyplot as plt
...
# Plot the pupil plane
amplitude = proper.prop_get_amplitude(wavefront)
...
proper.prop_lens(wavefront, 24 * diam, "Focal Plane")
proper.prop_propagate(wavefront, 24 * diam, "Focal Plane")

# Plot the focal plane
plt.figure(figsize=(12,8))
plt.imshow((proper.prop_get_amplitude(wavefront))**(1/4), origin = "lower", cmap = plt.cm.gray)
plt.title("Focal Plane", fontsize = 18)
plt.show()

# Multiply by FPM
fpm_s = proper.prop_get_sampling(wavefront)
fpm_real_map = proper.prop_readmap(wavefront, fpm_real, 0, 0, AMPLITUDE=True, SAMPLING=fpm_s/10)
fpm_imag_map = proper.prop_readmap(wavefront, fpm_imag, 0, 0, AMPLITUDE=True, SAMPLING=fpm_s/10)
fpm_real = np.fft.fftshift(fpm_real_map)
fpm_imag = np.fft.fftshift(fpm_imag_map)
fpm_complex = fpm_real + 1j * fpm_imag
proper.prop_multiply(wavefront, fpm_complex)

# Plot the focal plane after the FPM
plt.figure(figsize=(12,8))
plt.imshow((proper.prop_get_amplitude(wavefront))**(1/4), origin = "lower", cmap = plt.cm.gray)
plt.title("Focal Plane - After FPM", fontsize = 18)
plt.show()

# Handle Lyot Stop
lyot_s = proper.prop_get_sampling(wavefront)
lyot_stop_map = proper.prop_readmap(wavefront, lyot_stop, 0, 0, AMPLITUDE=True, SAMPLING=lyot_s)

# CRITICAL FIX: Shift the Lyot Stop map to [0,0] center
# Without ifftshift, the stop's center is at [512, 512], but the beam's center is at [0,0]
lyot_stop_shifted = np.fft.ifftshift(lyot_stop_map)
proper.prop_multiply(wavefront, lyot_stop_shifted)

# Plot the Lyot plane after the Lyot stop
plt.figure(figsize=(12,8))
plt.imshow((proper.prop_get_amplitude(wavefront))**(1/4), origin = "lower", cmap = plt.cm.gray)
...
# Plot the final image plane
plt.figure(figsize=(12,8))
plt.imshow(np.log10(proper.prop_get_amplitude(wavefront)), origin = "lower", cmap = plt.cm.gray)
plt.title("Final Image Plane", fontsize = 18)
plt.show()
"""
...
proper.prop_propagate(wavefront, 24 * diam, "Lyot Plane")
proper.prop_lens(wavefront, 24 * diam, "Lyot Plane")
proper.prop_propagate(wavefront, 24 * diam, "Lyot Plane")

# Plot the Lyot plane
plt.figure(figsize=(12,8))
plt.imshow((proper.prop_get_amplitude(wavefront))**(1/4), origin = "lower", cmap = plt.cm.gray)
plt.title("Lyot Plane", fontsize = 18)
plt.show()

# Multiply by Lyot stop
lyot_s = proper.prop_get_sampling(wavefront)
lyot_stop_map = proper.prop_readmap(wavefront, lyot_stop, 0, 0, AMPLITUDE=True, SAMPLING=lyot_s)
proper.prop_multiply(wavefront, lyot_stop_map)
